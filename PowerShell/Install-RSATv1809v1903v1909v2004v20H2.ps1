...
if (($WindowsBuild -eq $1809Build) -OR ($WindowsBuild -eq $1903Build) -OR ($WindowsBuild -eq $1909Build) -OR ($WindowsBuild -eq $2004Build) -OR ($WindowsBuild -eq $20H2Build)) {
	...
	if ($useWUServer -eq 1) {
            Write-Log -Message "***********************************************************"
            Write-Log -Message "Temporarily disabling WSUS in order to successfully install features on demand"
            Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate\AU" -Name "UseWuServer" -Value 0
            Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate" -Name "DisableWindowsUpdateAccess" -Value 0 -ErrorAction SilentlyContinue
...
# Reset WSUS Setting
            ...
            Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\WindowsUpdate" -Name "DisableWindowsUpdateAccess" -Value 1 -ErrorAction SilentlyContinue
            Restart-Service wuauserv
            Write-Log -Message "Restart Windows Update Services"
            Write-Log -Message "************************************************************"